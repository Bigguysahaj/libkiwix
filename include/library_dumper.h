...
{
 public:
  LibraryDumper(const Library* const library, const NameMapper* const NameMapper);
  ~LibraryDumper();
   ...
   * 
   * @param library pointer to a kiwix::Library
   * @param libraryId id for library
   */
  kainjow::mustache::list getCategoryList() const;
   ...
   * @param libraryId id for library
   */
  kainjow::mustache::list getLanguageList() const;

  /**
   * Get the human friendly title
   * 
   * @param title 
   */
  std::string humanFriendlyTitle(std::string title) const;
